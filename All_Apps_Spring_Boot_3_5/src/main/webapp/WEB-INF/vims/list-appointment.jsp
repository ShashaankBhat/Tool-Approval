...
<html lang="en">
  <head>
    <title>VISITOR DASHBOARD<%@include file="../includeJSP/company_name_title.jsp"%></title>
  ...
  <body class="sidebar-mini layout-navbar-fixed sidebar-collapse pace-primary">
            ...
            <div class="row mb-2">
              <div class="col-sm-6">
                <h5 class="font-3 font-weight-bold">
                  VISITOR DASHBOARD&ensp;
                  <a title="Book Appointment" href="book-appointment" type="button" class="btn btn-sm sequence-bg-2">
                    <i class="nav-icon fas fa-calendar-check text-white"></i>&emsp;
                    Book Appointment
                  </a>
                </h5>
              </div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  ...
                  <li class="breadcrumb-item font-3 active">Visitor Dashboard</li>
                </ol>
              </div>
            </div>
          </div>
		      ...
		      <div class="col-md-12">
		        <div class="card card-secondary">
		          <!-- <div class="card-header bg-3">
		            <h5 class="card-title font-weight-bold">List of Appointment</h5>
		            <div class="card-tools font-weight-bold">
		              <a title="Book Appointment" href="book-appointment" type="button" class="btn btn-tool text-white font-weight-bold">
		                <i class="nav-icon fas fa-calendar-check text-white"></i>&emsp;
		                Book Appointment
		              </a>
		            </div>
		              ...
		              <div class="col-md-12">
		                <div class="card card-primary collapsed-card">
		                  ...
		                  <div class="card-body" style="display: none;">
		                        <form modelAttribute="visitor" action="list-appointment" id="listFilterForm" method="post">
		                        <div class="row">
		                          <div class="col-md-3 form-group">
		                            <label class="font-1">Date</label>
		                            <input style="float: right" name="meetingStartDateTemp" type="date" id="filterDate" class="form-control form-" value="${today}" min="${today}"/>
		                          </div>
		                          <c:if test="${itAdmin eq 1 || vimsReception eq 1 }">
		                            <div class="col-md-5 form-group">
		                              <label class="font-2">Parent Entity</label>
		                              <select name="visitingLocationTemp" id="locationFilter" class="form-control">
		                                <option value="-1"> Select Location </option>
		                                <c:forEach items="${userEntityMap}" var="userEntity" varStatus="userEntityLoop">
		                                  <c:if test="${visitor.visitingLocationTemp eq userEntity.entityInfoForUsers.mEntityId }">
		                                  ...
		                                  <c:if test="${visitor.visitingLocationTemp ne userEntity.entityInfoForUsers.mEntityId }">
		            ...
		            <div class="row">
		              <div class="col-md-12 tableOverFlowDiv">
		                <table class="table table-bordered table-striped dataTable" id="visitorTable">
		                  ...
		                  <tbody>
		                    <c:forEach items="${visitorList}" var="visitor" varStatus="visitorLoop">
		                      <tr class="bg-table-row-${visitorLoop.index %2 }">
		                        <td>${visitor.actionForList }</td>
		                        <td><fmt:formatDate pattern = "dd-MMM-yyyy" value = "${visitor.visitorCreatedOn }" /></td>
		                        ...
		                        <td>${visitor.visitingLocation.entityAcronym } - ${visitor.visitingLocation.locationId.locationName }</td>
		                      </tr>
		                    </c:forEach>
		                  </tbody>
		                  <tfoot>
    ...
    <script src="usedStaticFiles/essentials/daterangepicker/tempusdominus-bootstrap-4.min.js"></script>
  	<script>
		...
		if(itAdmin == 1 || vimsAdmin == 1 || vimsReception == 1){
			$("#filterDate").removeAttr('min');
		}
	</script>
  </body>
</html>