...
@Data
@Entity
@Table(name = "ta_approval_matrix_history")
public class Ta_Approval_Matrix_History implements Comparable<Ta_Approval_Matrix_History> {

	@Override
    public int compareTo(Ta_Approval_Matrix_History matrix) {
        return matrix.getTaApprovalMatrixHistoryId().compareTo(this.taApprovalMatrixHistoryId);
    }

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "ta_approval_matrix_history_id")
    private Long taApprovalMatrixHistoryId;

    @OneToOne
    @JoinColumn(name = "ta_approval_matrix_id", nullable = false)
    private Ta_Approval_Matrix taApprovalMatrixId;
