...
@Service
public class VMS_M_Visitor_Type_Service_Impl implements VMS_M_Visitor_Type_Service {

	@Autowired private VMS_M_Visitor_Type_Dao mVisitorTypeDao;
	@Autowired private VMS_M_Visitor_Type_History_Dao mVisitorTypeHistoryDao;
	@Autowired private M_User_Dao mUserDao;
	@Autowired private InputValidation inputVaildationService;

	@Override
	public List<VMS_M_Visitor_Type> getVisitorTypeList(HttpServletRequest request, Integer offset) {
		List<VMS_M_Visitor_Type> finalVisitorTypeList = new ArrayList<VMS_M_Visitor_Type>();
		try {
			List<VMS_M_Visitor_Type> visitorTypeList = null;
	...
	@Override
	public List<VMS_M_Visitor_Type> getVisitorTypeByActiveStatus(Integer visitorTypeActive) {
		List<VMS_M_Visitor_Type> visitorTypeList = null;
		try {
			System.out.println("111");
			visitorTypeList = mVisitorTypeDao.findByVisitorTypeActive(visitorTypeActive);
	...
	@Override
	@Transactional
	public String saveVisitorTypeDetails(HttpServletRequest request, VMS_M_Visitor_Type visitorType) {
		...
		if(visitorType.getMVisitorTypeId() != null) {
			VMS_M_Visitor_Type currVisitorType = mVisitorTypeDao.findById(Long.parseLong(""+visitorType.getMVisitorTypeId())).get();
			Integer proceedFlag = 0;
			List<VMS_M_Visitor_Type> visitorTypeExists = mVisitorTypeDao.findByVisitorTypeName(visitorType.getVisitorTypeName());
			if(visitorTypeExists != null && visitorTypeExists.size() != 0) {
				for(VMS_M_Visitor_Type visitorTypeDetails : visitorTypeExists) {
			...
			if(proceedFlag.compareTo(0) == 0) {
				...
				currVisitorType.getVisitorTypeActive().compareTo(visitorType.getVisitorTypeActive()) == 0) {
					...
					VMS_M_Visitor_Type_History visitorTypeHistory = new VMS_M_Visitor_Type_History();
	...
	public String getVisitorTypeDetails(HttpServletRequest request) {
		...
		try {
			/*VMS_M_Visitor_Type visitorType = mVisitorTypeDao.getOne(Integer.parseInt(""+request.getParameter("mVisitorTypeId")));*/
			VMS_M_Visitor_Type visitorType = mVisitorTypeDao.findById(Long.parseLong(""+request.getParameter("mVisitorTypeId"))).get();
