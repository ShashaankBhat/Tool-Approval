var pagination = true;
$(function () {
	$("#taStatusTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#taStatusTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_taStatusTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
		   	if(index % 2 == 0) {
		   		$(row).addClass("bg-table-row-0");
		   	} else {
		   		$(row).addClass("bg-table-row-1");
		   	}
		   },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"App List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "Application Hub",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#taStatusTable_wrapper .col-md-6:eq(0)');
	$('select[name="taStatusTable_length"]').val(25).trigger('change');
	var taStatusTable = $("#taStatusTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(taStatusTable.rows().count() < 25 || loadAll > 1) {
		$("#loadMore_taStatusTable").hide();
		pagination = false;
	}
	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditAppModal").attr("onclick", "location.href='app'");
		$("#editAppModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function editStatus(id) {
    $.ajax({
        url: 'getStatusDetails',
        type: 'GET',
        data: { taStatusId: id },
        dataType: 'json',
        success: function (result) {
            $("#taStatusId").val(id);
            $("#taStatusNameEdit").val(result.taStatusName);
            $("#taStatusDescriptionEdit").val(result.taStatusDescription);
            if (result.taStatusActive == "1") $("#taStatusActiveEdit1").prop("checked", true);
            else $("#taStatusActiveEdit2").prop("checked", true);

            $("#editTaStatusModal").modal({ backdrop:'static', keyboard:false });
        }
    });
}
$("#taStatus").submit(function(){
	$("#addStatus").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	var submitOrNot = true;
	if(taStatusNameValidation("") == false) {
		submitOrNot = false;
	}
	if(taStatusDescriptionValidation("") == false) {
		submitOrNot = false;
	}
	if($("#taStatusActive1").prop("checked") == false && $("#taStatusActive2").prop("checked") == false) {
		$("#isActiveError").html("Status state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#addStatus").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".package-loader").hide();
		return false;
	}
});
$("#taStatusEdit").submit(function(){
	$("#editStatus").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	var submitOrNot = true;
	if(taStatusNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if(taStatusDescriptionValidation("Edit") == false) {
		submitOrNot = false;
	}
	if($("#taStatusActiveEdit1").prop("checked") == false && $("#taStatusActiveEdit2").prop("checked") == false) {
		$("#isActiveEditError").html("Role state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#editStatus").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".package-loader").hide();
		return false;
	}
});
$("#taStatusName").focusout(function() {
	taStatusNameValidation("");
});
$("#taStatusNameEdit").focusout(function() {
	taStatusNameValidation("Edit");
});
function taStatusNameValidation(idAppend) {
	var id = "taStatusName"+idAppend;
	var error = "taStatusNameError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var taStatusName = $("#"+id).val().replace(/\s+/g, " ");
	if(taStatusName != "") {
		if(centralAlphaNumericCheck.test(taStatusName) == false) {
			$("#"+error).html("Status Name cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Status Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$("#taStatusDescription").focusout(function() {
	taStatusDescriptionValidation("");
});
$("#taStatusDescriptionEdit").focusout(function() {
	taStatusDescriptionValidation("Edit");
});
function taStatusDescriptionValidation(idAppend) {
	var id = "taStatusDescription"+idAppend;
	var error = "taStatusDescriptionError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var taStatusDescription = $("#"+id).val().replace(/\s+/g, " ");
	if(taStatusDescription != "") {
		if(centralAlphaNumericCheck.test(taStatusDescription) == false) {
			$("#"+error).html("Status Description cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
	} else {
		$("#"+error).html("Status Description cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});
function loadNextSetValues() {
	var taStatusTable = $("#taStatusTable").DataTable();
	var offsetValue = taStatusTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	$.ajax({
		url:'loadMoreStatus',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				taStatusTable.row.add([
					result[i]["statusAction"],result[i]["statusName"],result[i]["statusDescription"],
					result[i]["statusActive"],result[i]["statusCreatedBy"],result[i]["statusCreatedOn"],
					result[i]["statusUpdatedBy"],result[i]["statusUpdatedOn"]
				]);
			}
			taStatusTable.draw(false);
			var currentPage = taStatusTable.page.info().page;
			var totalPages = taStatusTable.page.info().pages;
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					taStatusTable.page(currentPage+1).draw(false);
				}
			}
			$("#taStatusTable_paginate").scroll();
			$('#taStatusTable_paginate').show();
			if(result.length < 25) {
				pagination = false;
				$('#loadMore_taStatusTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
		},
		error:function() {
			$('#taStatusTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}