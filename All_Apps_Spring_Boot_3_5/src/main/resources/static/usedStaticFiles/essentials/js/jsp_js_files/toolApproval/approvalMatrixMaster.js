var pagination = true;

$(function () {

    $("#approvalMatrixTable").DataTable({
        "ordering": false,
        "paging": true,
        "info": true,
        "lengthChange": true,
        "searching": true,
        "autoWidth": false,
        "responsive": true,
        "createdRow": function (row,data,index) {
            if(index % 2 == 0) {
                $(row).addClass("bg-table-row-0");
            } else {
                $(row).addClass("bg-table-row-1");
            }
        },
        "buttons": ["csv", "excel", {
            extend: 'pdf',
            className:'buttons-pdf',
            text:'PDF',
            titleAttr: 'Download in PDF',
            title:"Approval Matrix List",
            orientation: 'portrait',
            customize: function (doc) {
                var now = new Date();
                var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
                var logo = 'data:image/png;base64,';
                doc['header']=(function() { return {
                    columns: [{
                        alignment: 'left',
                        fontSize: 15,
                        text: "ANAND Projects",
                    }, {
                        alignment: 'right',
                        image: logo,
                        width: 100,
                        height:20
                    }],
                    margin: 10
                }});
                doc['footer']=(function(page, pages) { return {
                    columns: [{
                        alignment: 'left',
                        text: ['Created on: ', { text: jsDate.toString() }]
                    }, {
                        alignment: 'right',
                        text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
                    }],
                    margin: 20
                }});
            },
            pageSize: 'LEGAL',
            exportOptions: {
                columns: ':visible thead th:not(.noExport)'
            }
        }, "print", "colvis"]
    }).buttons().container().appendTo('#approvalMatrixTable_wrapper .col-md-6:eq(0)');

});


/* ================= EDIT MODAL ================= */

function editApprovalMatrix(id) {

    $('body').addClass('disableClickBody');
    $(".package-loader").show();

    $.ajax({
        url: 'getApprovalMatrixDetails',
        type: 'GET',
        data: { 'mApprovalMatrixId': id },
        dataType: 'json',
        success: function (result) {

            // ID
            $("#mApprovalMatrixId").val(id);

            // Entity (FIXED ID)
            $("#mEntityIdEdit").val(result.entityId);

            // Approver Levels
			$("#approverLevel1Edit").val(result.approverLevel1Id);
			$("#approverLevel2Edit").val(result.approverLevel2Id);
			$("#approverLevel3Edit").val(result.approverLevel3Id);
			$("#approverLevel4Edit").val(result.approverLevel4Id);


            // Positions

            // Active Flag (FIXED ID)
            if (result.matrixActive == "1") {
                $("#matrixActiveEdit1").prop("checked", true);
            } else {
                $("#matrixActiveEdit2").prop("checked", true);
            }

            $('body').removeClass('disableClickBody');
            $(".package-loader").hide();

            $("#editTaApprovalMatrixModal").modal({
                backdrop: 'static',
                keyboard: false
            });

        },
        error: function () {
            $('body').removeClass('disableClickBody');
            $(".package-loader").hide();
            alert("Unable to fetch details.");
        }
    });
}

$("#approvalMatrix").submit(function () {

    var isValid = true;

    function validateSelect(id, message) {
        var field = $("#" + id);
        field.removeClass("is-invalid");
        field.next(".invalid-feedback").text("");

        if (!field.val()) {
            field.addClass("is-invalid");
            field.next(".invalid-feedback").text(message);
            isValid = false;
        }
    }

    validateSelect("mEntityId", "Entity is mandatory.");
    validateSelect("approverLevel1", "Approver Level 1 is mandatory.");
    validateSelect("approverLevel2", "Approver Level 2 is mandatory.");
    validateSelect("approverLevel3", "Approver Level 3 is mandatory.");
    validateSelect("approverLevel4", "Approver Level 4 is mandatory.");

    if ($("#isActiveDiv input[name='matrixActive']:checked").length === 0) {
        isValid = false;
        $("#matrixActiveError").text("Please select Active/Inactive status.");
    } else {
        $("#matrixActiveError").text("");
    }

    return isValid;
});

$("#approvalMatrixEdit").submit(function () {

    var isValid = true;

    function validateSelect(id, message) {
        var field = $("#" + id);
        field.removeClass("is-invalid");
        field.next(".invalid-feedback").text("");

        if (!field.val()) {
            field.addClass("is-invalid");
            field.next(".invalid-feedback").text(message);
            isValid = false;
        }
    }

    validateSelect("mEntityIdEdit", "Entity is mandatory.");
    validateSelect("approverLevel1Edit", "Approver Level 1 is mandatory.");
    validateSelect("approverLevel2Edit", "Approver Level 2 is mandatory.");
    validateSelect("approverLevel3Edit", "Approver Level 3 is mandatory.");
    validateSelect("approverLevel4Edit", "Approver Level 4 is mandatory.");

    if ($("#isActiveEditDiv input[name='matrixActive']:checked").length === 0) {
        isValid = false;
        $("#matrixActiveEditError").text("Please select Active/Inactive status.");
    } else {
        $("#matrixActiveEditError").text("");
    }

    return isValid;
});

$("select").change(function () {
    $(this).removeClass("is-invalid");
});

$("input[type='radio']").change(function () {
    $(".invalid-feedback").text("");
});
