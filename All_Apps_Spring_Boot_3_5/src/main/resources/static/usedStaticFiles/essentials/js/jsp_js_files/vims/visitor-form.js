...
function editApp(mAppId) {
	...
	$.ajax({
		...
		success: function(result) {
			...
			$("#appLinkEdit").val(result["appLink"]);
			$("#appDescriptionEdit").val(result["appDescription"]);
			$("#appDescription2Edit").val(result["appDescription2"]);
...
function appNameValidation(idAppend) {
	...
	if(appName != "") {
		if(centralAlphaNumericCheck.test(appName) == false) {
...
$("#appLink").focusout(function() {
	appLinkValidation("");
});
$("#appLinkEdit").focusout(function() {
	appLinkValidation("Edit");
});
var appLinkCheck = /^[ A-Za-z0-9-Á& ]*$/;
function appLinkValidation(idAppend) {
	var id = "appLink"+idAppend;
	var error = "appLinkError"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var appLink = $("#"+id).val().replace(/\s+/g, " ");
	if(appLink != "") {
		if(centralAlphaNumericCheck.test(appLink) == false) {
			$("#"+error).html("App Link cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
		...
		$("#"+error).html("App Link cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
...
function appDescriptionValidation(idAppend) {
	...
	if(appDescription != "") {
		if(centralTextCheck.test(appDescription) == false) {
...
$("#appDescription2").focusout(function() {
	appDescription2Validation("");
});
$("#appDescription2Edit").focusout(function() {
	appDescription2Validation("Edit");
});
function appDescription2Validation(idAppend) {
	var id = "appDescription2"+idAppend;
	var error = "appDescription2Error"+idAppend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var appDescription2 = $("#"+id).val().replace(/\s+/g, " ");
	if(appDescription2 != "") {
		if(centralTextCheck.test(appDescription2) == false) {
			$("#"+error).html("App Description 2 cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
		...
		$("#"+error).html("App Description 2 cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
