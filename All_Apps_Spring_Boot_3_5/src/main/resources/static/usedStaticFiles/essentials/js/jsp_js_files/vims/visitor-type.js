var pagination = true;
$(function() {
	$("#visitorTypeTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#visitorTypeTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_visitorTypeTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"VisitorType List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#visitorTypeTable_wrapper .col-md-6:eq(0)');

	var visitorTypeTable = $("#visitorTypeTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(visitorTypeTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_visitorTypeTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditVisitorTypeModal").attr("onclick", "location.href='visitor-type'");
		$("#editVisitorTypeModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var visitorTypeTable = $("#visitorTypeTable").DataTable();
	var offsetValue = visitorTypeTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	$.ajax({
		url:'loadMoreVisitorType',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				visitorTypeTable.row.add([
					result[i]["VisitorTypeAction"],result[i]["VisitorTypeName"],
					result[i]["VisitorTypeDescription"],result[i]["VisitorTypeActive"],
					result[i]["VisitorTypeCreatedBy"],result[i]["VisitorTypeCreatedOn"],
					result[i]["VisitorTypeUpdatedBy"],result[i]["VisitorTypeUpdatedOn"]
				]);
			}
			visitorTypeTable.draw(false);
			var currentPage = visitorTypeTable.page.info().page;
			var totalPages = visitorTypeTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					visitorTypeTable.page(currentPage+1).draw(false);
				}
			}
			$("#visitorTypeTable_paginate").scroll();
			$('#visitorTypeTable_paginate').show();
			if(result.length < 20) {
				pagination = false;
				$('#loadMore_visitorTypeTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
		},
		error:function() {
			$('#visitorTypeTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editVisitorType(mVisitorTypeId) {
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	$.ajax({
		url: 'getVisitorTypeDetails',
		type: 'GET',
		data: { 'mVisitorTypeId': mVisitorTypeId },
		dataType: 'json',
		success: function(result) {
			clearEditFormValidation();
			$("#mVisitorTypeId").val(mVisitorTypeId);
			$("#visitorTypeNameEdit").val(result["visitorTypeName"]);
			if(result["visitorTypeActive"] == "1") {
				$("#visitorTypeActiveEdit1").prop('checked', true);
			} else {
				$("#visitorTypeActiveEdit2").prop('checked', true);
			}
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
			$("#editVisitorTypeModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
		}
	});
}
function clearEditFormValidation() {
	$("#VisitorTypeNameEdit").html("");
	$("#VisitorTypeNameEdit").removeClass("is-valid");
	$("#VisitorTypeNameEdit").removeClass("is-invalid");
	
	$("#isActiveEditError").html("");
	$("#isActiveEditError").removeClass("is-invalid");
	$("#isActiveEditError").removeClass("is-valid");
}
$("#visitorType").submit(function() {
	$("#submitVisitorTypeButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	var submitOrNot = true;
	if(visitorTypeNameValidation("") == false) {
		submitOrNot = false;
	}
	if($("#visitorTypeActive1").prop("checked") == false && $("#visitorTypeActive2").prop("checked") == false) {
		$("#isActiveError").html("Visitor Type state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitVisitorTypeButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".package-loader").hide();
		return false;
	}
});
$("#visitorTypeEdit").submit(function(){
	$("#saveVisitorTypeChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	var submitOrNot = true;
	if(visitorTypeNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveVisitorTypeChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".package-loader").hide();
		return false;
	}
});
$("#visitorTypeName").focusout(function() {
	visitorTypeNameValidation("");
});
$("#visitorTypeNameEdit").focusout(function() {
	visitorTypeNameValidation("Edit");
});
var VisitorTypeNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function visitorTypeNameValidation(idVisitorTypeend) {
	var id = "visitorTypeName"+idVisitorTypeend;
	var error = "visitorTypeNameError"+idVisitorTypeend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var visitorTypeName = $("#"+id).val().replace(/\s+/g, " ");
	if(visitorTypeName != "") {
		if(centralAlphaNumericCheck.test(visitorTypeName) == false) {
			$("#"+error).html("Visitor Type cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
		return true;
	} else {
		$("#"+error).html("Visitor Type Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});