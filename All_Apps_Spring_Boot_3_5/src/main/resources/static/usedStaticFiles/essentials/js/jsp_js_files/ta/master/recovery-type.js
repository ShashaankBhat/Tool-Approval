var pagination = true;
$(function () {
	$("#taRecoveryTypeTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#taRecoveryTypeTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_taRecoveryTypeTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
		   	if(index % 2 == 0) {
		   		$(row).addClass("bg-table-row-0");
		   	} else {
		   		$(row).addClass("bg-table-row-1");
		   	}
		   },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Recovery Type List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "Tool Approval",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#taRecoveryTypeTable_wrapper .col-md-6:eq(0)');
	$('select[name="taRecoveryTypeTable_length"]').val(25).trigger('change');
	var taRecoveryTypeTable = $("#taRecoveryTypeTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(taRecoveryTypeTable.rows().count() < 25 || loadAll > 1) {
		$("#loadMore_taRecoveryTypeTable").hide();
		pagination = false;
	}
	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
		if ($("#activenessTemp").val() == 1) {
			$("#taRecoveryTypeActive1").prop("checked", true);
		} else {
			$("#taRecoveryTypeActive2").prop("checked", true);
		}
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditTaRecoveryTypeModal").attr("onclick", "location.href='ta-business-unit'");
		if ($("#activenessTemp").val == 1) {
			$("#taRecoveryTypeActiveEdit1").prop("checked", true);
		} else {
			$("#taRecoveryTypeActiveEdit2").prop("checked", true);
		}
		$("#editTaRecoveryTypeModal").modal({
			backdrop: 'static',
			keyboard: false
		});
	}
});
function editRecoveryType(id) {
    $.ajax({
        url: 'getRecoveryTypeDetails',
        type: 'GET',
        data: { 'taRecoveryTypeId': id },
        dataType: 'json',
        success: function (result) {
            $("#taRecoveryTypeId").val(id);
            $("#taRecoveryTypeNameEdit").val(result.taRecoveryTypeName);
            $("#taRecoveryTypeDescriptionEdit").val(result.taRecoveryTypeDescription);
            if (result.taRecoveryTypeActive == "1") {
                $("#taRecoveryTypeActiveEdit1").prop("checked", true);
            } else {
                $("#taRecoveryTypeActiveEdit2").prop("checked", true);
            }
            $("#editTaRecoveryTypeModal").modal({
                backdrop: 'static',
                keyboard: false
            });
        },
        error: function () {
            alert("Unable to fetch details.");
        }
    });
}
$("#taRecoveryType").submit(function () {
    var isValid = true;
    var name = $("#taRecoveryTypeName");
    name.removeClass("is-invalid");
    $("#taRecoveryTypeNameError").text("");
    if (!name.val()) {
        name.addClass("is-invalid");
        $("#taRecoveryTypeNameError").text("Business Unit Name is mandatory.");
        isValid = false;
    }
    var description = $("#taRecoveryTypeDescription");
    description.removeClass("is-invalid");
    $("#taRecoveryTypeDescriptionError").text("");
    if (!description.val()) {
        description.addClass("is-invalid");
        $("#taRecoveryTypeDescriptionError").text("Business Unit Description is mandatory.");
        isValid = false;
    }
    if ($("input[name='taRecoveryTypeActive']:checked").length === 0) {
        $("#isActiveError").text("Please select Active/Inactive status.");
        isValid = false;
    } else {
        $("#isActiveError").text("");
    }
    return isValid;
});
$("#taRecoveryTypeEdit").submit(function () {
    var isValid = true;
    var name = $("#taRecoveryTypeNameEdit");
    name.removeClass("is-invalid");
    $("#taRecoveryTypeNameErrorEdit").text("");
    if (!name.val()) {
        name.addClass("is-invalid");
        $("#taRecoveryTypeNameErrorEdit").text("Business Unit Name is mandatory.");
        isValid = false;
    }
    var description = $("#taRecoveryTypeDescriptionEdit");
    description.removeClass("is-invalid");
    $("#taRecoveryTypeDescriptionErrorEdit").text("");
    if (!description.val()) {
        description.addClass("is-invalid");
        $("#taRecoveryTypeDescriptionErrorEdit").text("Business Unit Description is mandatory.");
        isValid = false;
    }
    if ($("#taRecoveryTypeActiveEdit1:checked, #taRecoveryTypeActiveEdit2:checked").length === 0) {
        $("#isActiveEditError").text("Please select Active/Inactive status.");
        isValid = false;
    } else {
        $("#isActiveEditError").text("");
    }
    return isValid;
});
$("#taRecoveryType input, #taRecoveryType textarea, #taRecoveryType select").on("input change", function () {
    $(this).removeClass("is-invalid");
    $("#taRecoveryTypeNameError").text("");
    $("#taRecoveryTypeDescriptionError").text("");
    $("#isActiveError").text("");
});
$("#taRecoveryTypeEdit input, #taRecoveryTypeEdit textarea, #taRecoveryTypeEdit select").on("input change", function () {
    $(this).removeClass("is-invalid");
    $("#taRecoveryTypeNameErrorEdit").text("");
    $("#taRecoveryTypeDescriptionErrorEdit").text("");
    $("#isActiveEditError").text("");
});

function loadNextSetValues() {
	var taRecoveryTypeTable = $("#taRecoveryTypeTable").DataTable();
	var offsetValue = taRecoveryTypeTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	$.ajax({
		url:'loadMoreRecoveryType',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				taRecoveryTypeTable.row.add([
					result[i]["recoveryTypeAction"],result[i]["recoveryTypeName"],result[i]["recoveryTypeDescription"],
					result[i]["recoveryTypeActive"],result[i]["recoveryTypeCreatedBy"],result[i]["recoveryTypeCreatedOn"],
					result[i]["recoveryTypeUpdatedBy"],result[i]["recoveryTypeUpdatedOn"]
				]);
			}
			taRecoveryTypeTable.draw(false);
			var currentPage = taRecoveryTypeTable.page.info().page;
			var totalPages = taRecoveryTypeTable.page.info().pages;
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					taRecoveryTypeTable.page(currentPage+1).draw(false);
				}
			}
			$("#taRecoveryTypeTable_paginate").scroll();
			$('#taRecoveryTypeTable_paginate').show();
			if(result.length < 25) {
				pagination = false;
				$('#loadMore_taRecoveryTypeTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
		},
		error:function() {
			$('#taRecoveryTypeTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}