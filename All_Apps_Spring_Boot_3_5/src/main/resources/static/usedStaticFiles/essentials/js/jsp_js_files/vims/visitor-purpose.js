var pagination = true;
$(function() {
	$("#visitorPurposeTable").DataTable({
		"ordering": false,
		"paging": true,
		"info": true,
		"lengthChange": true,
		"searching": true,
		"autoWidth": false,
		"responsive": true,
		"drawCallback": function(settings) {
			if (pagination) {
				$('#visitorPurposeTable_paginate').find(".pagination").append('<li class="paginate_button page-item"><a onclick="loadNextSetValues()" id="loadMore_visitorPurposeTable" class="fa fa-arrow-right page-link" href="#" title="Load More"></a></li>');
			}
		},
		"createdRow": function (row,data,index) {
	    	if(index % 2 == 0) {
	    		$(row).addClass("bg-table-row-0");
	    	} else {
	    		$(row).addClass("bg-table-row-1");
	    	}
	    },
		"buttons": ["csv", "excel", {
			extend: 'pdf',
			className:'buttons-pdf',
			text:'PDF',
			titleAttr: 'Download in PDF',
			title:"Visitor Purpose List",
			orientation: 'portrait',
			customize: function (doc) {
				var now = new Date();
				var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
				var logo = 'data:image/png;base64,';
				doc['header']=(function() { return {
					columns: [{
						alignment: 'left',
						fontSize: 15,
						text: "ANAND Projects",
					}, {
						alignment: 'right',
						image: logo,
						width: 100,
						height:20
					}],
					margin: 10
				}});
				doc['footer']=(function(page, pages) { return {
					columns: [{
						alignment: 'left',
						text: ['Created on: ', { text: jsDate.toString() }]
					}, {
						alignment: 'right',
						text: ['page ', { text: page.toString() },	' of ',	{ text: pages.toString() }]
					}],
					margin: 20
				}});
			},
			pageSize: 'LEGAL',
			exportOptions: {
				columns: ':visible thead th:not(.noExport)'
			}
		}, "print", "colvis"]
	}).buttons().container().appendTo('#visitorPurposeTable_wrapper .col-md-6:eq(0)');

	var visitorPurposeTable = $("#visitorPurposeTable").DataTable();
	var loadAll = window.location.href.split("all").length;
	if(visitorPurposeTable.rows().count() < 20 || loadAll > 1) {
		$("#loadMore_visitorPurposeTable").hide();
		pagination = false;
	}

	if($("#openAddModel").val() != "") {
		$(".btn-tool").click();
	}
	if($("#openEditModel").val() != "") {
		$("#closeEditVisitorPurposeModal").attr("onclick", "location.href='visitor-purpose'");
		$("#editVisitorPurposeModal").modal({
		    backdrop: 'static',
		    keyboard: false
		});
	}
});
function loadNextSetValues() {
	var visitorPurposeTable = $("#visitorPurposeTable").DataTable();
	var offsetValue = visitorPurposeTable.rows().count();
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	$.ajax({
		url:'loadMoreVisitorPurpose',
		type:'GET',
		data:{'offsetValue':offsetValue},
		dataType : 'json',
		success: function(result) {
			for(var i=0; i<result.length; i++) {
				visitorPurposeTable.row.add([
					result[i]["visitorPurposeAction"],result[i]["visitorPurposeName"],
					result[i]["visitorPurposeDescription"],result[i]["visitorPurposeActive"],
					result[i]["visitorPurposeCreatedBy"],result[i]["visitorPurposeCreatedOn"],
					result[i]["visitorPurposeUpdatedBy"],result[i]["visitorPurposeUpdatedOn"]
				]);
			}
			visitorPurposeTable.draw(false);
			var currentPage = visitorPurposeTable.page.info().page;
			var totalPages = visitorPurposeTable.page.info().pages;
			console.log(totalPages);
			if(totalPages > 1) {
				if(currentPage != (totalPages-1)) {
					visitorPurposeTable.page(currentPage+1).draw(false);
				}
			}
			$("#visitorPurposeTable_paginate").scroll();
			$('#visitorPurposeTable_paginate').show();
			if(result.length < 20) {
				pagination = false;
				$('#loadMore_visitorPurposeTable').hide();
			}
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
		},
		error:function() {
			$('#visitorPurposeTable_paginate').show();
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
			alert("Something went wrong, kindly try again after sometime.");
		}
	});
}
function editVisitorPurpose(mVisitorPurposeId) {
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	$.ajax({
		url: 'getVisitorPurposeDetails',
		type: 'GET',
		data: { 'mVisitorPurposeId': mVisitorPurposeId },
		dataType: 'json',
		success: function(result) {
			clearEditFormValidation();
			$("#mVisitorPurposeId").val(mVisitorPurposeId);
			$("#visitorPurposeNameEdit").val(result["visitorPurposeName"]);
			if(result["visitorPurposeActive"] == "1") {
				$("#visitorPurposeActiveEdit1").prop('checked', true);
			} else {
				$("#visitorPurposeActiveEdit2").prop('checked', true);
			}
			$('body').removeClass('disableClickBody');
			$(".package-loader").hide();
			$("#editVisitorPurposeModal").modal({
			    backdrop: 'static',
			    keyboard: false
			});
		},
		error: function() {
			alert("Something went wrong, try again later");
		}
	});
}
function clearEditFormValidation() {
	$("#VisitorPurposeNameEdit").html("");
	$("#VisitorPurposeNameEdit").removeClass("is-valid");
	$("#VisitorPurposeNameEdit").removeClass("is-invalid");
	
	$("#isActiveEditError").html("");
	$("#isActiveEditError").removeClass("is-invalid");
	$("#isActiveEditError").removeClass("is-valid");
}
$("#visitorPurpose").submit(function() {
	$("#submitVisitorPurposeButton").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	var submitOrNot = true;
	if(visitorPurposeNameValidation("") == false) {
		submitOrNot = false;
	}
	if($("#visitorPurposeActive1").prop("checked") == false && $("#visitorPurposeActive2").prop("checked") == false) {
		$("#isActiveError").html("Visitor Type state must be selected");
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#submitVisitorPurposeButton").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".package-loader").hide();
		return false;
	}
});
$("#visitorPurposeEdit").submit(function(){
	$("#saveVisitorPurposeChanges").addClass("disabledButton");
	$('body').addClass('disableClickBody');
	$(".package-loader").show();
	var submitOrNot = true;
	if(visitorPurposeNameValidation("Edit") == false) {
		submitOrNot = false;
	}
	if(submitOrNot) {
		return true;
	} else {
		$("#saveVisitorPurposeChanges").removeClass("disabledButton");
		$('body').removeClass('disableClickBody');
		$(".package-loader").hide();
		return false;
	}
});
$("#visitorPurposeName").focusout(function() {
	visitorPurposeNameValidation("");
});
$("#visitorPurposeNameEdit").focusout(function() {
	visitorPurposeNameValidation("Edit");
});
var VisitorPurposeNameCheck = /^[ A-Za-z0-9-Á& ]*$/;
function visitorPurposeNameValidation(idVisitorPurposeend) {
	var id = "visitorPurposeName"+idVisitorPurposeend;
	var error = "visitorPurposeNameError"+idVisitorPurposeend;
	$("#"+id).removeClass("is-valid");
	$("#"+id).removeClass("is-invalid");
	$("#"+error).html("");
	var visitorPurposeName = $("#"+id).val().replace(/\s+/g, " ");
	if(visitorPurposeName != "") {
		if(centralAlphaNumericCheck.test(visitorPurposeName) == false) {
			$("#"+error).html("Visitor Type cannot contain special characters.");
			$("#"+id).addClass("is-invalid");
			return false;
		} else {
			$("#"+id).addClass("is-valid");
			return true;
		}
		return true;
	} else {
		$("#"+error).html("Visitor Type Name cannot be empty.");
		$("#"+id).addClass("is-invalid");
		return false;
	}
}
$('#isActiveDiv input[type="radio"]').click(function() {
	$("#isActiveError").html("");
});
$('#isActiveEditDiv input[type="radio"]').click(function() {
	$("#isActiveEditError").html("");
});